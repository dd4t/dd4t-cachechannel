...
public abstract class BaseTextJMSCacheChannelConnector extends JMSCacheChannelConnector {

    private static final Logger LOG = LoggerFactory.getLogger(BaseTextJMSCacheChannelConnector.class);

    @Override
    public void configure(Configuration configuration) throws ConfigurationException {
        LOG.info("Loading BaseTextJMSCacheChannelConnector");
        ...
        if (("AsyncJMS11".equals(strategy)) || ("AsyncJMS11MDB".equals(strategy))) {
            this.client = getTextJMS11Approach(jndiContextProperties, topicConnectionFactoryName, topicName, "AsyncJMS11MDB".equals(strategy));
        } else if ("SyncJMS11".equals(strategy)) {
            this.client = getSynchronousJMS11Approach(jndiContextProperties, topicConnectionFactoryName, topicName);
        } else if (("AsyncJMS10".equals(strategy)) || ("AsyncJMS10MDB".equals(strategy))) {
            this.client = getTextJMS10Approach(jndiContextProperties, topicConnectionFactoryName, topicName, "AsyncJMS10MDB".equals(strategy));
        } else {
            throw new ConfigurationException("Unknown 'Strategy':" + strategy + " for the JMS Connector");
        }
    }

    protected abstract TextJMS11Approach getTextJMS11Approach(Properties jndiProperties, String factoryName, String topicName, boolean isMDBMode);
    protected abstract SynchronousJMS11Approach getSynchronousJMS11Approach(Properties jndiProperties, String factoryName, String topicName);
    protected abstract TextJMS10Approach getTextJMS10Approach(Properties jndiProperties, String factoryName, String topicName, boolean isMDBMode);
    ...
    public class TextJMS11Approach extends JMSCacheChannelConnector.JMS11Approach {
        ...
        public void connect(MessageListener messageListener, ExceptionListener exceptionListener) throws JMSException, NamingException {
            ...
            if (!this.isMDBMode) {
                ...
                {
                    BaseTextJMSCacheChannelConnector.LOG.error("setExceptionListener failed. Most likely due to container restrictions. In these environments the MDB com.tridion.cache.JMSBean must be setup instead", e);
                }
            }
            ...
            if (!this.isMDBMode) {
                ...
                {
                    BaseTextJMSCacheChannelConnector.LOG.error("setMessageListener failed. Most likely due to container restrictions. In these environments the MDB com.tridion.cache.JMSBean must be setup instead", e);
                }
            }
